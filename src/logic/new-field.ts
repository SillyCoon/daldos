...
export class NewField {
  ...
  static initial(size = 16): NewField {
    const snapshot = new FieldSnapshot(NotationConverter.initialNotation(size));
    return NewField.restore(snapshot);
  }

  static restore(snapshot: FieldSnapshot): NewField {
  ...
  *iterateFromToExcludeFirst(
  ...
  equals(otherField: NewField) {
    return this.makeSnapshot() === otherField.makeSnapshot();
  }

  activate(figureCoordinate: Coordinate, currentColor: Color): NewField {
  ...
  moveFigure(from: Coordinate, to: Coordinate): NewField {
  ...
  replaceFigure(coord: Coordinate, figure: Figure | null): NewField {
